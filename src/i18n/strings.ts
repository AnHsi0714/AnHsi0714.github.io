export interface Strings {
  ...
  gallery: {
    ...
    tags: Record<
      'click-regenerate' | 'drag-draw' | 'keyboard-game' | 'button-game' | 'drag-physics' | 'static',
      string
    >
    hints: Record<
      'click-regenerate' | 'drag-draw' | 'keyboard-game' | 'button-game' | 'drag-physics',
      string
    >
  }
  dreams: {
...
export const strings: Record<'zh' | 'en', Strings> = {
  zh: {
    ...
    friends: {
      ...
      backendNotConfigured: '後端尚未設定（缺 Supabase 環境變數），暫時無法載入朋友作品。',
      loading: '載入朋友作品中…',
      loadFailed: '載入失敗：',
      emptyTitle: '尚無朋友創作',
      ...
      prev: '上一個作品',
      next: '下一個作品',
      goto: (index: number, nickname: string) => `前往第 ${index} 個作品：${nickname}`,
    ...
    creator: {
      ...
      pageTitle: '畫一個作品',
      backendNotConfigured: '後端尚未設定（缺 Supabase 環境變數），暫時無法提交作品。',
      updatedTitle: '作品已更新！',
      submittedTitle: '作品已送出！',
      thanksEdit: (nickname: string) => `謝謝你，${nickname}，創作牆上的作品已經換成新的這張了。`,
      thanksCreate: (nickname: string) => `謝謝你，${nickname}，你的作品已經掛上創作牆了。`,
      viewWall: '去創作牆看看',
      gateSubtitle: '輸入邀請碼和暱稱就能開始創作；已經用過的邀請碼可以重新編輯之前的作品。',
      asIdentity: (nickname: string) => `以「${nickname}」的名義創作。`,
      editIdentity: '修改邀請碼／暱稱',
      editModeNotice: '這個邀請碼已經用過，現在是編輯模式——送出後會覆蓋你原本的作品。',
      ...
      introLabel2d: '作品、個人敘述 or 其他（選填，別人點開你的作品時會顯示，嚴禁不當內容）',
  ...
  en: {
    ...
    home: {
      ...
      titleEn: '鄭安琋',
      tagline: 'CS background × Visualization research × Frontend engineering',
    ...
    gallery: {
      ...
      hints: {
        ...
        'keyboard-game': 'Move with arrow keys / WASD, click on-screen buttons and options to start',
        'button-game': 'Click START to begin; click START again after each round',
    ...
    friends: {
      ...
      subtitle: "2D pixel art and 3D creatures made by friends using invite codes.",
      iHaveCode: 'I have an invite code',
      backendNotConfigured: "Backend isn't configured yet (missing Supabase environment variables), so friends' work can't load right now.",
      ...
      prev: 'Previous piece',
      next: 'Next piece',
      goto: (index: number, nickname: string) => `Go to piece ${index}: ${nickname}`,
    ...
    creator: {
      ...
      backendNotConfigured: "Backend isn't configured yet (missing Supabase environment variables), so submissions aren't available right now.",
      updatedTitle: 'Piece updated!',
      submittedTitle: 'Piece submitted!',
      thanksEdit: (nickname: string) => `Thanks, ${nickname} — the piece on the wall has been replaced with this new one.`,
      thanksCreate: (nickname: string) => `Thanks, ${nickname} — your piece is now up on the wall.`,
      viewWall: 'See the wall',
      gateSubtitle: "Enter an invite code and a nickname to start creating; an already-used code lets you re-edit your previous piece.",
      asIdentity: (nickname: string) => `Creating as "${nickname}".`,
      editIdentity: 'Edit invite code / nickname',
      editModeNotice: "This invite code has already been used — you're in edit mode now, and submitting will overwrite your original piece.",
      chooseKind: "Choose a creation type (can't be changed once you start)",
      kind2d: '2D Pixel Art',
      kind3d: '3D Creature Painting',
      chooseGridSize: "Choose a canvas size (can't be changed once you start drawing)",
      color: 'Color',
      brush: 'Brush',
      fill: 'Fill',
      erase: 'Erase',
      ...
      introLabel2d: 'A description or note about your piece (optional, shown when others open it — no inappropriate content)',
      introPlaceholder2d: "Something you'd like to say to whoever sees this",
      introPlaceholder3d: "Something you'd like to say to whoever sees this creature",
      submitFailed: 'Submission failed: ',
      submitting: 'Submitting…',
      update: 'Update Piece',
      submit: 'Submit Piece',
      paint: 'Paint',
      dragToRotateHint: 'Left-click drag to paint, right-click drag to rotate, scroll to zoom.',
    },
  },
}
