...
| 區塊               | 格式建議                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 文章（含讀書心得） | 每篇一個 `.md`，放在 `content/articles/<zh\|en>/<book\|journal\|paper>/<slug>.md`（先分語言再分類別，中英同 slug 各自成檔，不再用 `.en.md` 副檔名）；frontmatter：`type: book\|paper\|tech\|journal`、標題/日期；`author`/`rating` 只有 book 才填；`excerpt` 可選，沒填就自動從正文萃取前 100 字；渲染為詳細頁 `/articles/:slug`，支援篩選器（標題、分類、評分、日期）              |
...
前端流程：朋友輸入邀請碼 + 暱稱 →（2D/3D 編輯器產出 `data`）→ 呼叫
...
**縮圖儲存**：兩種類型都不需要產生/儲存縮圖圖檔，`data` 本身就是完整的「圖」：

- **2D**：前端直接拿 `pixels` 陣列在一個小 `<canvas>` 上重繪即可，省掉 base64/Storage 那一層。即使列表上同時顯示幾十個朋友作品，重繪像素方塊的成本也遠低於載入圖片。
- **3D**（原本預設「改走 3D 就需要預渲染縮圖 + Storage bucket」，實作後推翻）：創作牆卡片用 IntersectionObserver **只在捲進可視範圍時**掛即時 Three.js 場景（慢速自轉展示），捲出去就卸載換回佔位——每隻怪獸只有約 300 顆 instanced 方塊、渲染成本很低，真正的限制是 WebGL context 數量（瀏覽器約 8~16 個），而可視範圍內同時最多只有 3~6 張卡，離上限很遠，預渲染縮圖因此不需要。
...
- 全版背景（延伸到 NavBar 後方、貼齊頁尾）沿用列表頁 `GalleryGrid.module.scss` 的 `margin-top: -2rem`（精確抵銷 `main` 的 `pt-8`，讓展場貼齊 sticky NavBar 底部）＋`margin-bottom: -2rem`（抵銷 `main` 的 `pb-8`）full-bleed 手法；兩者缺一都會導致背景沒貼齊視窗邊緣，或底部露出一截 `main` 預設背景色可以被捲動到。`min-height` 額外扣掉 `NavBar.tsx` 用 `ResizeObserver` 即時量出來的 `--nav-h`，避免內容不夠一頁高時逼出頁面垂直捲軸（NavBar 是 `sticky` 且會 `flex-wrap` 換行，淨空不是固定值）。
- 因為 `.stage` 用了上述負 margin 手法，`position: absolute` 的返回連結（`.back`）量測基準點也跟著往上位移了 2rem（藏到 sticky NavBar 底下），`top` 必須再加回那 2rem 才會落在 NavBar 下方實際可見的區域，否則會被 NavBar 蓋住。
...
## 7. 朋友 2D/3D 創作功能：編輯器設計

已定案（2026-07-12）：**2D 像素風與 3D 怪獸塗色並存**，朋友在邀請碼驗證後自選類型（選定即鎖，二次編輯不能換類型）。原本「最終只會二選一、不同時維護兩套」的假設在 3D 做成「固定形狀、只塗色」之後推翻——3D 的維護成本收斂到一份共用形狀資料＋一個塗色編輯器，兩種類型的 `data` 都是稀疏座標陣列，資料層完全共用同一張表與同一組 RPC（`kind` 欄位與 check constraint 從 0001 就有，並存上線資料庫零改動）。
...
- **怪獸形狀是全站共用資產**（`lib/creatureBody.ts`，用 `/dev/creature-builder` 堆積木工具手雕、匯出座標貼回），朋友**只能塗色、不能改形狀**——創作自由度收斂在配色上，不需要存模型檔，`data` 大小與內容風險都跟 2D 同級。
- 整隻怪獸拆成 8 個可上色區域（`REGIONS`：軀幹/頭/角/尾巴/四腳，四腳形狀共用樣板但塗色各自獨立），`data jsonb` 一樣採「只存塗過色的格子」的稀疏陣列：

  ```json
  {
    "colors": [
      { "region": "torso", "x": -3, "y": 2, "z": -1, "color": "#33dbdb" }
    ]
  }
  ```

- **編輯器（`Creator3DEditor.tsx`）**：左鍵拖曳塗色、右鍵轉視角、滾輪縮放；工具只有自由選色/清除/清空；undo/redo 與 2D 編輯器同一套 stroke-batched reducer 模式（一次拖曳算一步）。未塗色格子顯示中性底色。
- **創作牆檢視**：卡片進可視範圍才掛即時 Three.js 場景（慢速自轉，見 §5 縮圖儲存備註），點擊置中卡片開整頁遮罩放大檢視（可拖曳旋轉/縮放，敘述附在下方）。唯讀渲染統一走 `components/VoxelPaintedCreature.tsx`（成功畫面縮圖/卡片/放大檢視三處共用），鏡頭對準點兼自轉軸心是從形狀資料動態算的質心（`creatureViewCenter`），重雕形狀不用手調鏡頭。

### 內容風險

低解析度像素網格本身就把「能畫出什麼」限制在很小的範圍內（即使自由選色，最壞情況也只是一張低解析度像素圖）；3D 塗色更受限（只能改固定形狀的顏色）。兩者都不需要額外的審核機制；`is_visible` 欄位還是留著，萬一真的有需要還是能手動關閉個別作品。
...
│   │   ├── dreams/
│   │   └── friends/
│   │       ├── Friends.tsx          # 創作牆（carousel + 3D 即時檢視/放大遮罩 + 邀請碼入口）
│   │       ├── InviteGate.tsx       # 邀請碼＋暱稱表單
│   │       ├── Creator.tsx          # gate → 選 2D/3D → 對應編輯器的流程殼
│   │       ├── Creator2DEditor.tsx  # 2D 像素編輯器，見 §7
│   │       └── Creator3DEditor.tsx  # 3D 怪獸塗色編輯器，見 §7
...
已決定（2026-07-12）：2D 像素與 3D 怪獸塗色**並存**（3D 走固定形狀、只塗色，見 §7），不再是二選一。
