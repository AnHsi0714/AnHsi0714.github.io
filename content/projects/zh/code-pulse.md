...
本研究採單組前後測設計，共 56 位受試者，分為高中組（三年級，已修過程式設計課程）與大學組（二年級，已修過資料結構與程式設計相關課程）。前後測分別使用 A、B 兩個版本的測驗，兩卷測量相同概念但題目不同，受試者依組別交替使用不同版本作為前測與後測，以降低受試者因重複接觸相同題目而產生的記憶效應。受試者可運用時間有限，操作平台約 20–30 分鐘後即進行後測與問卷調查。
...
相較之下，學習信心在兩組皆達統計顯著（大學組 p < 0.001；高中組 p = 0.021），顯示受試者在使用平台後，對演算法學習的信心有所提升。此結果初步顯示，動態視覺化可能有助於提升學習者對演算法概念的理解信心與學習意願。
...
本研究採單組前後測設計，缺乏控制組，因此無法排除測驗熟悉度、短期學習與新奇效應等因素的干擾，測驗表現與學習信心的提升不能直接歸因於平台介入本身。後續研究可加入控制組並延長操作時間，進一步驗證平台的學習成效。
...
使用者研究也點出一個值得深思的落差：介面設計帶來的心理效益（學習信心顯著提升）跟可量測的學習成效（測驗分數僅呈正向趨勢）之間並不完全一致。短時間的視覺化操作似乎能有效降低學習焦慮，但要轉化成可量測的知識習得，可能還需要更長的練習時間、控制組設計或更精準的引導。
