...
四個範例的 pipe 內容完全一樣，只有 operator 換了名字，這也是這張對照表想表達的重點：決定行為的不是 request 怎麼寫，而是攤平策略選哪一個。
...
| --- | --- | --- | --- |
| map | 把每個值轉換成新的值，一對一映射 | API 回來的資料要轉成畫面需要的格式 | `map((res) => res.data)` |
| filter | 只放行符合條件的值 | 只想要串流裡符合某個條件的資料 | `filter((n) => n > 0)` |
| tap | 不改變資料，只是在中間做一次 side effect（例如 log） | 想在資料流過的路上記錄或除錯，但不想影響資料本身 | `tap((val) => console.log(val))` |
| distinctUntilChanged | 跟前一個值相同就不繼續往下傳 | 避免同一個值重複觸發後續邏輯 | `distinctUntilChanged()` |
| catchError | 攔截錯誤，換成一個新的 Observable，讓串流不會直接中斷 | API 請求失敗時不想讓整條串流死掉，想接住錯誤繼續運作 | `catchError(() => of(fallback))` |
| retry | 發生錯誤時自動重新訂閱原本的 Observable，重試幾次 | 網路請求偶爾失敗，想在真的顯示錯誤前先自動重試 | `retry(3)` |
| forkJoin | 等所有傳入的 Observable 都完成，一次拿到全部的最終結果 | 需要同時打好幾支 API，等全部回來才能顯示畫面 | `forkJoin([getUser(), getOrders()])` |
| take(1) | 只取第一個值，拿到後自動取消訂閱 | 只需要訂閱一次的資料，不需要持續監聽 | `take(1)` |
| takeUntil | 持續訂閱，直到另一個 Observable 發出值為止，之後自動取消訂閱 | 元件銷毀時要取消所有訂閱 | `takeUntil(this.destroy$)` |
